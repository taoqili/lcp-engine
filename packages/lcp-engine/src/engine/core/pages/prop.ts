...
import Context, { SetterProvider } from '../../context';
import { DISPLAY_TYPE, IPropConfig, ISetterConfig } from '../bundle/prototype';
import CompositeProp from './compositeProp';
import { Node } from './node';
import { Props } from './props';

const { uniqueId, cloneDeep, isDataEqual, combineInitial, Transducer } = require('@ali/ve-utils');
 ...
 */
let dynamicSetterProvider: SetterProvider;
...
export default class Prop implements IVariableSettable {
  ...
  constructor(parent: any, config: IPropConfig, data?: any) {
    ...
    this.id = uniqueId(null, 'prop', 'engine-prop');
    ...
    dynamicSetterProvider = new Context().getPlugin('ve.plugin.setterProvider');
  ...
  public init(defaultValue?: any) {
    ...
    this.i18nLink = I18nUtil.attach(this, this.value,
      (val: any) => { this.setValue(val, false, true); });
  ...
  public sync() {
    ...
    if (sync) {
      const value = sync.call(this, this.getValue(true), this.hotValue);
      if (value !== undefined) {
        this.setValue(value);
      }
    } else {
  ...
  public isSupportVariable() {
    return this.config.supportVariable;
  }
  ...
  public setUseVariable(flag?: boolean) {
  ...
  public setValue(value: any, isHotValue?: boolean, force?: boolean, extraOptions?: ISetValueOptions) {
    ...
    this.i18nLink = I18nUtil.attach(this, this.value, (val: any) => this.setValue(val, false, true));
    ...
    if (this.modify(force)) {
      ...
      if (mutator && !extraOptions.disableMutator) {
        mutator.call(this, this.value, this.hotValue, preValue, preHotValue);
      }
  ...
  public setHotData(hotData: IMMap<string, IHotDataMap>, options?: ISetValueOptions) {
    ...
    if (!options || !options.disableMutator) {
      const mutator = this.config.mutator;
      if (mutator) {
        mutator.call(this, value, hotValue, preValue, preHotValue);
      }
    }
  ...
  public getSetter() {
    ...
    if (typeof setterConfig === 'function' && !(setterConfig.prototype instanceof React.Component)) {
      return setterConfig.call(this, this.getValue());
    }
  ...
  public getSetterData(): ISetterConfig {
