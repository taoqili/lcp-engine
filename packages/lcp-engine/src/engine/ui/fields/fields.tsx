...
import SelectSetter from './setterSelector';
...
export class BlockField extends VEField {
  ...
  public renderHead() {
    ...
    return [
      <span className='engine-field-title' key={title}>{title}</span>,
...
export class AccordionField extends VEField {
  ...
  public componentWillUnmount() {
    if (this.willDetach) { this.willDetach(); }
  }

  public renderHead() {
    const { title, tip, toggleExpand, propName } = this.props;
    return (
      <div className='engine-field-head' onClick={() => toggleExpand()}>
  ...
  private _generateClassNames(props: IVEFieldProps) {
    this.classNames = ['engine-setting-field', 'engine-accordion-field',
...
export class EntryField extends VEField {
  ...
  public render() {
    ...
    const innerElements = [
      <span className='engine-field-title' key='field-title'>{title}</span>,
      renderTip(tip, { propName }),
  ...
  public render() {
    ...
    if (stage.getTabs) {
      ...
      if (stage.hasTabs()) {
        ...
        tabs = (<div className='engine-settings-tabs'>{
      ...
      if (currentTab) {
        if (currentTab.getVisibleItems) {
          // 这里需要处理composite下套group的情况
          let items = currentTab.getVisibleItems();
          // 用于收集在 group 里的 prop 的 id
          ...
          items = items.filter((item: any) => {
            ...
            if (groupItemIds.includes(item.getId())) {
              return false;
            }
            return true;
          });
          content = items.map((item: any) => <SettingField
            key={item.getId()}
            selected={selected}
            prop={item}
          />);
        } else if (currentTab.getSetter) {
          content = (<SettingField
            key={currentTab.getId()}
            selected={selected}
            prop={currentTab}
            forceDisplay='plain'
          />);
    ...
    if (stage.hasBack()) {
      ...
      stageBacker = (<div className='engine-settings-stagebacker' data-stage-target='stageback'>
        <Icons name='arrow' className='engine-field-arrow' size='12px' />
    ...
    return (
      <div ref={(ref) => { this.shell = ref; }} className={className}>
        {stageBacker}
        {tabs}
